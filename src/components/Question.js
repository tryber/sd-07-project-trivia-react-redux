...
class Questions extends Component {
  ...
  componentDidMount() {
    const { tokenObj, name, emailGravatar } = this.props;
    const questionsQuantity = 5;
    const gameState = { player: { name, assertions: 0, score: 0, emailGravatar } };
    localStorage.setItem('state', JSON.stringify(gameState));
    const { seconds } = this.props;
    if (seconds === 0) this.handleUniqueAnswer('incorrect');
    questionsAPI(questionsQuantity, tokenObj)
  ...
  handleUniqueAnswer(type) {
    const { seconds, addStop, addScore, name, emailGravatar,
...
const mapStateToProps = (state) => ({
  tokenObj: state.tokenReducer.token,
